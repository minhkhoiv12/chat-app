...
module.exports.userRegister = (req, res) => {
  ...
  // Check for missing fields, invalid email, password match, etc.
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    if (error.length > 0) {
      ...
      // Step 3: Process image upload (rename and copy)
        ...
        if (checkUser) {
          ...
          fs.copyFile(files.image.filepath, newPath, async (error) => {
            if (!error) {
              ...
              // Step 7: Send response with token in cookie
              const options = {
                expires: new Date(
